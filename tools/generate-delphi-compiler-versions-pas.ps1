...
# This loop is data-driven: adding a new version to the JSON automatically adds its
# $IFDEF block here with the correct array index.
$hasVER185Init = @($sorted | Where-Object { [string]$_.verDefine -eq 'VER185' }).Count -gt 0
...
for ($i = 0; $i -lt $sorted.Count; $i++) {
  ...
  if ($hasVER185Init -and $verDefine -eq 'VER180') {
    $null = $sb.AppendLine('  {$IFDEF VER180}{$IFNDEF VER185}')
    $null = $sb.AppendLine('    CurrentDelphiCompilerVersion := DelphiVersions[' + $i + '];')
    $null = $sb.AppendLine('    IsCurrentDelphiCompilerVersionKnown := True;')
    $null = $sb.AppendLine('  {$ENDIF}{$ENDIF}')
  } else {
    $null = $sb.AppendLine('  {$IFDEF ' + $verDefine + '}')
    $null = $sb.AppendLine('    CurrentDelphiCompilerVersion := DelphiVersions[' + $i + '];')
    $null = $sb.AppendLine('    IsCurrentDelphiCompilerVersionKnown := True;')
    $null = $sb.AppendLine('  {$ENDIF}')
  }
